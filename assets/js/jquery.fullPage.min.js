	...
	b.fn.fullpage = function(c) {
		function W(a) {
			if (c.autoScrolling) {
				...
				a = b(".section.active");
				if (!n && !r) if (f = K(f), v = f.y, z = f.x, a.find(".slides").length && Math.abs(A - z) > Math.abs(w - v)) Math.abs(A - z) > b(window).width() / 100 * c.touchSensitivity && (A > z ? a.find(".controlArrow.next:visible").trigger("click") : a.find(".controlArrow.prev:visible").trigger("click"));
				else if (a = a.find(".slides").length ? a.find(".slide.active").find(".scrollable") : a.find(".scrollable"), Math.abs(w - v) > b(window).height() / 100 * c.touchSensitivity) if (w > v) if (0 < a.length) if (B("bottom", a)) b.fn.fullpage.moveSectionDown();
				else
				return !0;
				else b.fn.fullpage.moveSectionDown();
				else if (v > w) if (0 < a.length) if (B("top", a)) b.fn.fullpage.moveSectionUp();
				else
				return !0;
		...
		function X(a) {
			c.autoScrolling && (a = K(a.originalEvent), w = a.y, A = a.x)
		}
		function p(a) {
			if (c.autoScrolling) {
				a = window.event || a;
				...
				var f;
				f = b(".section.active");
				if (!n) if (f = f.find(".slides").length ? f.find(".slide.active").find(".scrollable") : f.find(".scrollable"), 0 > a) if (0 < f.length) if (B("bottom", f)) b.fn.fullpage.moveSectionDown();
				else
				return !0;
				else b.fn.fullpage.moveSectionDown();
				else if (0 < f.length) if (B("top", f)) b.fn.fullpage.moveSectionUp();
				else
				return !0;
				else b.fn.fullpage.moveSectionUp();
		...
		function k(a, f, d) {
			...
			if ("undefined" !== typeof g) {
				...
				n = !0;
				"undefined" !== typeof s && L(p, r, s);
				c.autoScrolling ? (e.top = -g, a = "#superContainer") : (e.scrollTop = g, a = "html, body");
				var k = function() {
					t && t.length && (d ? b(".section:first").before(t) : b(".section:last").after(t), x(b(".section.active").position().top))
				};

				c.css3 && c.autoScrolling ? (b.isFunction(c.onLeave) && c.onLeave.call(this, m, l), M("translate3d(0px, -" + g + "px, 0px)", !0), setTimeout(function() {
					k();

					b.isFunction(c.afterLoad) && c.afterLoad.call(this, s, E + 1);
					setTimeout(function() {
						n = !1;
						b.isFunction(f) && f.call(this)
					}, N)
				}, c.scrollingSpeed)) : (b.isFunction(c.onLeave) && c.onLeave.call(this, m, l), b(a).animate(e, c.scrollingSpeed, c.easing, function() {
					k();
		...
		function y(a, f) {
			var d = f.position(),
				...
				h = l.index(".section"),
				k = l.data("anchor"),
				p = l.find(".fullPage-slidesNav"),
				m = f.data("anchor"),
				n = F;
			if (c.onSlideLeave) {
				var q = l.find(".slide.active").index(),
					t;
				t = q > g ? "left" : "right";
				n || b.isFunction(c.onSlideLeave) && c.onSlideLeave.call(this, k, h + 1, q, t)
			}
			f.addClass("active").siblings().removeClass("active");
			"undefined" === typeof m && (m = g);
			l.hasClass("active") && (c.loopHorizontal || (l.find(".controlArrow.prev").toggle(0 != g), l.find(".controlArrow.next").toggle(!f.is(":last-child"))), L(g, m, k));
			c.css3 ? (d = "translate3d(-" + d.left + "px, 0px, 0px)", a.find(".slidesContainer").addClass("easing").css({
				"-webkit-transform" : d, "-moz-transform": d,
				"-ms-transform": d,
				transform: d
			}), setTimeout(function() {
				n || b.isFunction(c.afterSlideLoad) && c.afterSlideLoad.call(this, k, h + 1, m, g);
				r = !1
				...
				n || b.isFunction(c.afterSlideLoad) && c.afterSlideLoad.call(this, k, h + 1, m, g);
				r = !1
			});p.find(".active").removeClass("active");p.find("li").eq(g).find("a").addClass("active")
		}
		function Q() {
			F = !0;
			...
			b(".section").each(function() {
				...
				c.verticalCentered && b(this).find(".tableCell").css("height", R(b(this)) + "px");
				b(this).css("height", h + "px");
				a = b(this).find(".slides");
				a.length && y(a, a.find(".slide.active"))
			});
		...
		function C(a) {
			...
			e > b ? d.length ? d.css("height", b + "px").parent().css("height", b + "px") : (c.verticalCentered ? a.find(".tableCell").wrapInner('<div class="scrollable" />') : a.wrapInner('<div class="scrollable" />'), a.find(".scrollable").slimScroll({
				height : b + "px", size: "10px",
				alwaysVisible: !0
		...
		function R(a) {
			var b = h;
			if (c.paddingTop || c.paddingBottom) b = a, b.hasClass("section") || (b = a.closest(".section")), a = parseInt(b.css("padding-top")) + parseInt(b.css("padding-bottom")), b = h - a;
			return b
		}
		function M(a, c) {
		...
		function G(a, c) {
			...
			a === q || d.hasClass("active") ? T(d, c) : k(d, function() {
				T(d, c)
			})
		}
		function T(a, b) {
		...
		function L(a, b, d) {
			...
			var e = "";
			c.anchors.length && (a ? ("undefined" !== typeof d && (e = d), "undefined" === typeof b && (b = a), H = b, location.hash = e + "/" + b) : ("undefined" !== typeof a && (H = b), location.hash = d))
		}
		function $() {
		...
		function x(a) {
			c.css3 ? M("translate3d(0px, -" + a + "px, 0px)", !1) : b("#superContainer").css("top", -a)
		}
		c = b.extend({
			verticalCentered: !0,
			resize: !0,
		...
		b.fn.fullpage.setMouseWheelScrolling = function(a) {
			a ? document.addEventListener ? (document.addEventListener("mousewheel", p, !1), document.addEventListener("DOMMouseScroll", p, !1)) : document.attachEvent("onmousewheel", p) : document.addEventListener ? (document.removeEventListener("mousewheel", p, !1), document.removeEventListener("DOMMouseScroll", p, !1)) : document.detachEvent("onmousewheel", p)
		};

		b.fn.fullpage.setAllowScrolling = function(a) {
			a ? (b.fn.fullpage.setMouseWheelScrolling(!0), I && (b(document).off("touchstart MSPointerDown").on("touchstart MSPointerDown", X), b(document).off("touchmove MSPointerMove").on("touchmove MSPointerMove", W))) : (b.fn.fullpage.setMouseWheelScrolling(!1), I && (b(document).off("touchstart MSPointerDown"), b(document).off("touchmove MSPointerMove")))
		};
		...
		var r = !1,
			I = navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry|Windows Phone)/),
			h = b(window).height(),
			n = !1,
			F = !1,
			q, H;
		...
		b(".section").each(function(a) {
			...
			if (c.navigation) {
				...
				u.find("ul").append('<li data-tooltip="' + a + '"><a href="#' + e + '"><span></span></a></li>')
			}
			if (0 < d) {
				var e = 100 * d,
					g = 100 / d;
				...
				"#fff" != c.controlArrowColor && (b(this).find(".controlArrow.next").css("border-color", "transparent transparent transparent " + c.controlArrowColor), b(this).find(".controlArrow.prev").css("border-color", "transparent " + c.controlArrowColor + " transparent transparent"));
				c.loopHorizontal || b(this).find(".controlArrow.prev").hide();

				c.slidesNavigation && Z(b(this), d);
				f.each(function(a) {
					a || b(this).addClass("active");
					b(this).css("width", g + "%");
					c.verticalCentered && S(b(this))
				})
			...
			if (a.length) {
				var f = b('[data-anchor="' + a + '"]');
				!c.animateAnchor && f.length && (x(f.position().top), b.isFunction(c.afterLoad) && c.afterLoad.call(this, a, f.index(".section") + 1), f.addClass("active").siblings().removeClass("active"))
			}
		...
		b(window).scroll(function(a) {
			//if (!c.autoScrolling) {
				...
				//if (!a.hasClass("active")) {
					...
					//O(e);
					//P(e, 0);
					//c.anchors.length && !n && (q = e, location.hash = e);
					//clearTimeout(U);
					//U = setTimeout(function() {
					//	J = !1
					//}, 100)
					var random = Math.floor((Math.random() * 12) + 1);
					b.fn.fullpage.moveTo(random);
				//}
			//}
		});
      ...
      b.fn.fullpage.moveTo(random);
			...
			// (0 < a.length || !a.length && (c.loopBottom || c.continuousVertical)) && k(a, null, !1)
		};

		b.fn.fullpage.moveTo = function(a, c) {
			var d = "",
				d = isNaN(a) ? b('[data-anchor="' + a + '"]') : b(".section").eq(a - 1);
			"undefined" !== c ? G(a, c) : 0 < d.length && k(d)
		};
		...
		b(window).on("hashchange", function() {
			if (!J) {
				var a = window.location.hash.replace("#", "").split("/"),
					b = a[0],
					a = a[1],
					c = "undefined" === typeof q,
					e = "undefined" === typeof q && "undefined" === typeof a;
				(b && b !== q && !c || e || !r && H != a) && G(b, a)
			}
		});

		b(document).keydown(function(a) {
			if (c.keyboardScrolling && !n) switch (a.which) {
				case 38:
				case 33:
